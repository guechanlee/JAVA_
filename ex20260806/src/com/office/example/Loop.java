package com.office.example;

import java.util.Scanner;

public class Loop {
	public static void main(String[] args) {
		
		/*
		 * for문 & while문
		 */
		
		//지역변수
		//     초기화     조건식   증감식
//		for (int i = 0; i < 11; i++) {
//			System.out.println("i: " + i);
//		}
//		
//		//전역변수
//		int i = 0;
//		for (; i < 11; i++) {
//			System.out.println("i: " + i);
//		}
		
//		for (int i = 0, j = 10; i < 11 && j > 5; i++, j--) {
//			System.out.printf("i: %d \t j: %d \n", i, j);
//		}
		
		/*
		 * K마트에서 고객 감사 이벤트를 한다.
		 * 5%할인 쿠폰 증정!
		 * 쿠폰 개수는 사용자가 원하는 만큼
		 * 단, 최대 쿠폰수는 1인당 5개
		 */
		
		Scanner scanner = new Scanner(System.in);
//		int couponNum = scanner.nextInt();
//		
//		for (int i = 0, j = 5; i < couponNum && j > 0; i++, j--) {
//			System.out.println("쿠폰 발행!");
//		}
//		
//		for문에서 전위와 후위는 상관없다 할당되는 것이 아닌 증강되는 것이기 때문
//		for (int i = 0; i < couponNum; i++) {
//		for (int i = 0; i < couponNum; ++i) {
//			if (i >= 5) break;
//			System.out.println("쿠폰 발행!");
//		}
//			
//		
		scanner.close();
		
//		반복문 처음에는 실수는 지향해야한다.
//		for (double d = 0.0; d < 1; d += 0.1) {
//			System.out.printf("d: %f \n", d);
//		}
//		
		
		// while문
		/*
		 * for문이 횟수에의한 반복   => 10회
		 * while문 조건에의한 반복  => 100kg => 70kg
		 */
		
//		int num = 10;
//		while (num > 0) {
//			System.out.println("조건식에 만족했어요.");
//			num--;
//			
//		while (true) {
//			System.out.println("조건식에 만족했어요.");
//			num--;
//			if (num <= 0) break;
//			
//		}
			
		// do ~ while
		// 조건식과 별개로 무조건 딱 한번은 실행한다.
		/*
		 * 1. 시음 차원에서 처음 1회 무료로 우유를 배송할께요.
		 * 2. 드셔보시고 맛 있으면 구독(유료)
		 * 3. 계속 먹다보니까 질렸어요. 그래서 구독 중단하고 싶어요.
		 */
		
		int condition = 0;
		do {
			System.out.println("우유 배송!");
			System.out.println("정기 배송 여부 결정!");
			System.out.println("1.계속구독    2.구독중지");
			condition = scanner.nextInt();
		} while(condition == 1);
		
		scanner.close();
		
			
		
		
	}
}
